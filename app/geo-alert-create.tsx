...
const DURATIONS = [24, 72, 168] as const;
const REMINDERS = [0, 1, 2] as const;
...
export default function GeoAlertCreateScreen() {
  ...
  const [durationHours, setDurationHours] = useState<24 | 72 | 168>(72);
  const [reminderCount, setReminderCount] = useState<0 | 1 | 2>(0);
  ...
  useEffect(() => {
    if (!validLocation) Alert.alert(language === "en" ? "Location missing" : "Sted mangler", language === "en" ? "The report needs a valid location before a geo alert can be previewed." : "Rapporten må ha et gyldig sted før geovarselet kan forhåndsvises.");
  ...
  }, [radiusM, durationHours, reminderCount, validLocation, center]);

  const calculate = async () => {
    if (!reportId || !validLocation) return;
    try {
      setBusy(true);
      const next = await previewGeoAlert({ reportId, radiusM, durationHours, reminderCount, populationDensityBand: "LOW" });
      setResult(next);
    } catch (e: any) {
  ...
  return <>
    ...
    <View style={styles.safe}>
      <PremiumHeader title={language === "en" ? "Preview geo alert" : "Forhåndsvis geovarsel"} subtitle={String(params.title || "")} onBack={() => router.back()} />
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.card}>
          <Text style={styles.h2}>{language === "en" ? "Duration" : "Varighet"}</Text>
          <View style={styles.chips}>{DURATIONS.map(v => <Chip key={v} active={durationHours === v} label={v === 24 ? "24 t" : v === 72 ? "3 dager" : "7 dager"} onPress={() => setDurationHours(v)} />)}</View>
          <Text style={styles.h2Small}>{language === "en" ? "Reminders" : "Påminnelser"}</Text>
          <View style={styles.chips}>{REMINDERS.map(v => <Chip key={v} active={reminderCount === v} label={String(v)} onPress={() => setReminderCount(v)} />)}</View>
        </View>
        <Pressable disabled={busy} style={[styles.primary, busy && styles.disabled]} onPress={calculate}>
        ...
        {result && <View style={styles.result}>
          ...
          <Text style={styles.warning}>{language === "en" ? "This is a test price. No payment or alert is sent in this step." : "Dette er en testpris. Ingen betaling eller varsling skjer i dette steget."}</Text>
        </View>}
      </ScrollView>
    </View>
  </>;
...
const styles = StyleSheet.create({
  safe:{flex:1,backgroundColor:theme.colors.bg},content:{padding:14,paddingBottom:40},center:{flex:1,alignItems:"center",justifyContent:"center",padding:24},muted:{color:theme.colors.muted,fontWeight:"700"},mapFrame:{height:260,borderRadius:18,overflow:"hidden",borderWidth:1,borderColor:theme.colors.border,backgroundColor:theme.colors.card},map:{flex:1},card:{marginTop:12,padding:15,borderRadius:18,borderWidth:1,borderColor:theme.colors.border,backgroundColor:theme.colors.card},h2:{fontSize:17,fontWeight:"900",color:theme.colors.text},h2Small:{marginTop:14,fontSize:14,fontWeight:"900",color:theme.colors.text},chips:{flexDirection:"row",flexWrap:"wrap",gap:7,marginTop:10},chip:{paddingHorizontal:11,paddingVertical:8,borderRadius:999,backgroundColor:"#F1F5F9",borderWidth:1,borderColor:"#E2E8F0"},chipActive:{backgroundColor:theme.colors.primary,borderColor:theme.colors.primary},chipText:{fontWeight:"900",fontSize:12,color:"#475569"},chipTextActive:{color:"#fff"},help:{marginTop:10,color:theme.colors.muted,fontWeight:"700",fontSize:12},primary:{marginTop:14,minHeight:50,borderRadius:14,backgroundColor:theme.colors.primary,alignItems:"center",justifyContent:"center"},primaryText:{color:"#fff",fontWeight:"900"},disabled:{opacity:.55},result:{marginTop:14,padding:18,borderRadius:18,backgroundColor:"#EFF6FF",borderWidth:1,borderColor:"#93C5FD"},resultEyebrow:{fontWeight:"900",fontSize:11,color:"#1D4ED8"},resultTitle:{marginTop:6,fontWeight:"900",fontSize:21,color:theme.colors.text},resultMeta:{marginTop:5,fontWeight:"700",color:theme.colors.muted},priceRow:{marginTop:16,paddingTop:14,borderTopWidth:1,borderTopColor:"#BFDBFE",flexDirection:"row",justifyContent:"space-between",alignItems:"center"},priceLabel:{fontWeight:"800",color:theme.colors.text},price:{fontWeight:"900",fontSize:23,color:"#1D4ED8"},warning:{marginTop:10,fontWeight:"700",fontSize:12,color:"#92400E"}
});
